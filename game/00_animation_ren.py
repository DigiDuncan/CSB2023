...
def __get_image_size(rwops):
    ...
    if imghdr.what(rwops.raw.name) == 'png':
        ...
        if check != 0x0d0a1a0a:
            raise Exception("Invalid PNG file: " + rwops.raw.name)

        width, height = struct.unpack('>ii', head[16:24])

    elif imghdr.what(rwops.raw.name) == 'jpeg':
        try:
            rwops.seek(0)
            size = 2
            ftype = 0
            ...
            raise Exception("Failed to read JPEG headers from file: " + rwops.raw.name)

    else:
        raise Exception("unsupported image type on file: " + rwops.raw.name)
