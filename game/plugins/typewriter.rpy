init python:
    ...
    def screen_typewriter(st, at, typewriter_text, size):
        if st > 2.0:
            return Text(typewriter_text, textalign = 0.5, size = size, outlines=[(absolute(10), "#000", absolute(0), absolute(0))]), 0.1
        else:
            d = Text(typewriter_text[:math.ceil((st / 2.0) * len(typewriter_text))], textalign = 0.5, size = size, outlines=[(absolute(10), "#000", absolute(0), absolute(0))])
            return d, 0.1
