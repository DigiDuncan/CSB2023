label archival:
    window hide
    show screen warning("The following scene is a major tonal shift.\nIt may be disconcerting to some viewers.", "Warning: potential existential dread.", "pussy_out_archival")
    pause
    window show
    stop music
    scene cs_room_2
    ...
    n "..."
    scene bedroom_old
    play music "<loop 0>facing_worlds.ogg" volume 0.5
    ...
    n "They all look like the same person, except one is wearing sunglasses, another a top hat, and the third is wearing a hazmat suit."
    ...
    nova "Nova?"
    nova "Nova Nova?"
    k199 "God, what the fuck are those things? Make them attack CS!"
    ...
    k207 "{i}Ah! Ahh! Fuck, get them off!"
    show nova3 at mid_left with vpunch
    k199 "{i}Kill them! Kill themmm! 17, you piece of shit, make them stop!"
    show nova1 at right with vpunch
    k174 "{i}Oh God, what have I done?! I can't stop them!"
...
label archival_finale:
    ...
    play music "<loop 0>take_trip.ogg" volume 0.7
    ...
    n "Stretching to every edge of the room are rows of hundreds of glowing tanks, all filled with the same cyan fluid."
    ...
    n "CS steps over to one of them, and peers inside to see if he can see anything."
    hide cs with moveoutright
    ...
    play sound "<loop 0>sfx_less_annoying_alarm_sound.ogg" loop volume 0.6
    ...
    n "CS takes the exit and follows the signs while sprinting down the hallway."
    ...
    n "A bigger sign that clarifies the station is here hangs above him. The final doors open to reveal a train station, where a tram car is waiting for him on the rail."
    hide cs with moveoutright
    ...
    play music "<loop 0>take_trip2.ogg" volume 0.5
    n "He gets in on time as the door slowly shuts, right before the guys can get in."
    show archival_19
    with dissolve    
    n "CS goes over to the console in the front of the tram and pulls the giant forward lever."
    scene archival_15
    show hart1 at left
    ...
    with dissolve
    n "CS looks ahead and takes a bit of a breather as the tram starts to gain speed."
    scene train_start
    show cs disappointed at center
    with dissolve
    pause 1.0
    ...
    pause 2.0
    play music "<loop 0>everybody_wants.ogg" volume 1.0
