...
class Attacks:
    ...
    GENERGY = Attack("Genergy", "Sip a bit of energy drink for healing.", heal_fighters, target_count = 1, target_type = "allies", mult = 1.2, cooldown = 2, accuracy = 100, ex = False)
...
class Fighters:
    ...
    TATE_EX = Fighter("{image=gui/dx_text.png} Tate EX", True, 1111, 11, 35, [Attacks.DAMAGE_SCREM, Attacks.REVERB_RECALL, Attacks.ECHO_BLAST], Image("images/characters/tate/tatesmug.png"), ai = AIType.AGGRO, display_name = "Tate EX") 
...
class DamageIndicator:
    ...
    def play(self):
        if self.indicator_type == "heal":
            renpy.sound.play("audio/ut/snd_power.ogg", channel = "sfx")
            ...
            renpy.sound.play("audio/ut/snd_chime.ogg", channel = "sfx")
            self.play_sound = False
