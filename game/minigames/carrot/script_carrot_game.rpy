...
init python:
    ...
    class CarrotGameDisplayable(renpy.Displayable):
        def __init__(self):
            ...
            # Carrot highway
            self.carrots = [START_BEAT]
            self.next_carrot = START_BEAT + 1
        ...
        def render(self, width, height, st, at):
            ...
            # Render space
            if not self.pressed_space_once or self.song_time > SONG_LENGTH + 1.0:
            ...
            if self.next_carrot <= (NUMBER_OF_BEATS - BEATS_TO_DROP) and (self.beat_time(self.next_carrot) - self.song_time) * BEAT_GAP + CARROT_OFFSET <= 1920:
            ...
            if (
                    ...
                    START_BEAT <= nearest
                    and current <= (NUMBER_OF_BEATS - BEATS_TO_DROP)
                ):
                # Miss!
                self.misses += 1
                self.is_fcing = False
            ...
            if self._song_time > SONG_LENGTH:
                if self.is_fcing:
                    self.win = "superb"
