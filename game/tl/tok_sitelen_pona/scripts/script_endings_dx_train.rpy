# TODO: Translation updated at 2026-06-05 11:32

# game/scripts/script_endings_dx_train.rpy:2
translate tok_sitelen_pona train_home_rich_thief_60c0eada:

    # "Placeholder for if you completed train route with recovered stolen money."
    "Placeholder for if you completed train route with recovered stolen money."

# game/scripts/script_endings_dx_train.rpy:7
translate tok_sitelen_pona train_home_rich_winner_14f462d3:

    # "Placeholder for if you completed train route with recovered Vegas winnings."
    "Placeholder for if you completed train route with recovered Vegas winnings."

# game/scripts/script_endings_dx_train.rpy:12
translate tok_sitelen_pona train_home_broke_f7e099a7:

    # "Placeholder for if you completed train route without any money."
    "Placeholder for if you completed train route without any money."

translate tok_sitelen_pona strings:

    # game/scripts/script_endings_dx_train.rpy:32
    old "You've unlocked a colorful new theme! Check it out in CSettings!"
    new "You've unlocked a colorful new theme! Check it out in CSettings!"

    # game/scripts/script_endings_dx_train.rpy:43
    old "Awa awawa? AAAAAA! You've unlocked Awawa Mode! Check it out in CSettings!"
    new "Awa awawa? AAAAAA! You've unlocked Awawa Mode! Check it out in CSettings!"

